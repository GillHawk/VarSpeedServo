...
// move at specified position (value in degrees)
// this method cannot be used together with wait(), waitAndDetach() or isMoving(), use method write(value, speed) instead
...
// move at specified position (value in microseconds)
// this method cannot be used together with wait(), waitAndDetach() or isMoving(), use method write(value, speed) instead
void VarSpeedServo::writeMicroseconds(int value)
{
  ...
  if( (channel >= 0) && (channel < MAX_SERVOS) )   // ensure channel is valid
  {
    ...
    // End of Extension for slowmove
    servos[channel].Pin.isActive = true; // attach
  }
}
...
/*
  ...
  this method can be used together with wait(), waitAndDetach() or isMoving()
*/
...
void VarSpeedServo::write(int value, uint8_t speed, bool waitToFinish) {
  write(value, speed);

  if (waitToFinish) { // block until the servo is at expected position
      wait();
  }
}
...
void VarSpeedServo::wait() {
  byte channel = this->servoIndex;
  if(! servos[channel].Pin.isActive) return;
  ...
  // empirical delay correction (after ticks reach the target value, it take some time for servo to actually reach the position)
  delay(30);
}

// to be used only with "write(value, speed)"
void VarSpeedServo::waitAndDetach() {
  wait();
  // make sure we reached the position before detach
  delay(30);
  // detach
  servos[this->servoIndex].Pin.isActive = false;
}
...
bool VarSpeedServo::isMoving() {
  byte channel = this->servoIndex;
  if(! servos[channel].Pin.isActive) return false;
